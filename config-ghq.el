...
(defun ghq-worktree-create (branch-name)
  ...
  (let* ((repo-root (ghq--repo-root))
    ...
    ;; Setup Claude settings and commands symlinks
    (ghq--setup-claude-symlinks worktree-path)
...
(defun ghq--setup-claude-symlinks (worktree-path)
  "Setup Claude settings and commands symlinks in WORKTREE-PATH."
  (let* ((claude-dir (expand-file-name ".claude" worktree-path))
         (shared-settings "~/src/.crewcapableai.shared/settings.local.json")
         (shared-commands "~/src/.crewcapableai.shared/.claude/commands"))
    ;; Create .claude directory
    (unless (file-directory-p claude-dir)
      (make-directory claude-dir t))
    ;; Symlink settings.local.json
    (let ((settings-link (expand-file-name "settings.local.json" claude-dir)))
      (when (and (file-exists-p (expand-file-name shared-settings))
                 (not (file-exists-p settings-link)))
        (make-symbolic-link (expand-file-name shared-settings) settings-link)))
    ;; Symlink entire commands directory
    (let ((commands-link (expand-file-name "commands" claude-dir)))
      (when (and (file-directory-p (expand-file-name shared-commands))
                 (not (file-exists-p commands-link)))
        (make-symbolic-link (expand-file-name shared-commands) commands-link)))))
